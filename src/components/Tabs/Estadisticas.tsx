...
export default function Estadisticas({ players, matches, selectedCategory, onTabChange }: EstadisticasProps) {
    ...
    <div id="statistics-tab" className="space-y-4 text-left">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-club-gradient-elements border border-white/10 rounded-2xl p-4 flex items-center gap-3.5 shadow-xl col-span-1">
          ...
          <div>
            <span className="block text-[10px] text-indigo-200/55 font-bold uppercase tracking-wider">Ganados (Ef.)</span>
            <div className="flex items-baseline gap-1.5">
              <strong className="text-xl font-black text-white font-mono">{partidosGanados}</strong>
      ...
      <div className="bg-club-gradient-elements border border-white/10 rounded-2xl p-5 shadow-2xl">
        ...
        {topScorersList.length > 0 ? (
          <div className={`grid grid-cols-1 ${topScorersList.length > 1 ? 'sm:grid-cols-2' : ''} gap-4`}>
            {topScorersList.map((player) => (
              <div key={player.id} className="flex items-center gap-4 bg-black/25 border border-white/10 rounded-xl p-3 hover:border-emerald-500/30 transition duration-150">
                <div className="relative">
                  <div className="w-12 h-12 rounded-full bg-emerald-500/10 border border-white/10 flex items-center justify-center text-emerald-400 overflow-hidden">
                    {player.fotoUrl ? (
                      <img 
                        src={player.fotoUrl} 
                        alt={player.nombre} 
                ...
                <div className="bg-black/25 border border-white/10 px-3 py-1 rounded-lg text-center shrink-0 min-w-[70px]">
                  <span className="text-[8px] text-indigo-300 font-bold uppercase block leading-none mb-0.5">Goles</span>
          ...
          <div className="text-center py-4">
            <p className="text-xs text-indigo-205 py-2">Aún no se han registrado goles de jugadoras para esta categoría esta temporada.</p>
          </div>
        )}
      </div>

      {/* Tabs Selector for Leaderboards */}
      <div className="flex bg-black/25 p-1.5 rounded-2xl border border-white/10 overflow-hidden shadow-inner font-sports-condensed">
        <button
          onClick={() => setActiveBoard('goles')}
          className={`flex-1 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition duration-150 cursor-pointer text-center ${
            activeBoard === 'goles' ? 'bg-emerald-500 text-neutral-950 shadow-md' : 'text-indigo-200 hover:text-white hover:bg-white/5'
          }`}
        >
          Goleadoras
        </button>
        <button
          ...
          className={`flex-1 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition duration-150 cursor-pointer text-center ${
            activeBoard === 'asistencias' ? 'bg-emerald-500 text-neutral-950 shadow-md' : 'text-indigo-200 hover:text-white hover:bg-white/5'
          }`}
        >
          Asistencias
        </button>
        <button
          ...
          className={`flex-1 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition duration-150 cursor-pointer text-center ${
            activeBoard === 'presencias' ? 'bg-emerald-500 text-neutral-950 shadow-md' : 'text-indigo-200 hover:text-white hover:bg-white/5'
          }`}
        >
          Partidos Jugados
        </button>
        <button
          ...
          className={`flex-1 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition duration-150 cursor-pointer text-center ${
            activeBoard === 'tarjetas' ? 'bg-emerald-500 text-neutral-950 shadow-md' : 'text-indigo-200 hover:text-white hover:bg-white/5'
          }`}
        >
          Tarjetas
        </button>
      ...
      <div className="bg-club-gradient-elements border border-white/10 rounded-2xl p-5 shadow-2xl space-y-4">
        ...
        {activeBoard === 'goles' && (
          <div className="space-y-4">
            ...
            {topScorers.length > 0 ? (
              ...
              <p className="text-xs text-indigo-200 text-center py-6">No hay registros de goles asignados todavía en partidos jugados.</p>
            )}
          </div>
        )}
        ...
        {activeBoard === 'asistencias' && (
          <div className="space-y-4">
            ...
            {topAssists.length > 0 ? (
              ...
              <p className="text-xs text-indigo-200 text-center py-6">No hay registros de asistencias de gol registrados en partidos.</p>
            )}
          </div>
        )}
        ...
        {activeBoard === 'presencias' && (
          <div className="space-y-4">
            ...
            {topPresences.length > 0 ? (
              ...
              <p className="text-xs text-indigo-200 text-center py-6">No hay jugadoras cargadas con historial de partidos.</p>
            )}
          </div>
        )}
            ...
            {topCards.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs divide-y divide-white/5">
                  <thead>
                    <tr className="text-indigo-200/50 font-bold uppercase text-[9px] tracking-wide border-b border-white/10">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {topCards.slice(0, 8).map((player, idx) => (
                      <tr key={player.id} className="hover:bg-white/5 transition-colors">
                        <td className="py-2.5 px-1 font-mono text-indigo-350">{idx + 1}º</td>
                        <td className="py-2.5 px-2 font-bold text-white">
                          {player.nombre} {player.apellido} <span className="text-[10px] text-indigo-200/50 font-normal">#{player.numeroCamiseta}</span>
                        </td>
                        <td className="py-2.5 px-2 text-center text-indigo-100 font-mono font-black">{player.tarjetaVerde}</td>
                        <td className="py-2.5 px-2 text-center text-amber-450 font-mono font-black">{player.tarjetaAmarilla}</td>
                        <td className="py-2.5 px-2 text-center text-rose-400 font-mono font-black">{player.tarjetaRoja}</td>
                        <td className="py-2.5 px-3 text-center text-emerald-400 font-bold font-mono">{player.points}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <p className="text-xs text-indigo-200 text-center py-6">¡Excelente disciplina! No hay tarjetas registradas en toda la plantilla.</p>
            )}
          </div>
        )}
      </div>
